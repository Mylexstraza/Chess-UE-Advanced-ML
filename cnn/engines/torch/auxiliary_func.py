...
def board_to_matrix_2(board: Board):
    # 8x8 is a size of the chess board.
    # 12 = number of unique pieces.
    # 13th board for legal moves (WHERE we can move)
    # maybe 14th for squares FROM WHICH we can move? idk
    ...
    for square, piece in piece_map.items():
        ...
        if piece.color:
            matrix[13, row, col] = piece_type # 14th board for white pieces
        else:
            matrix[14, row, col] = piece_type # 15th board for black pieces
...
def create_input_for_nn_2(games):
    X = []
    y = []
    for game in games:
        board = game.board()
        for move in game.mainline_moves():
            X.append(board_to_matrix_2(board))
            y.append(move.uci())
            board.push(move)
    return np.array(X, dtype=np.float32), np.array(y)
